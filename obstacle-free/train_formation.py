...
class GNNAgent:
    def __init__(self, env, gamma=0.99, gae_lambda=0.95,
        ...
        # 网络参数 - 观测维度改为5（移除了障碍物信息）
        obs_dim = 5  # 修改后的观测维度：自身位置(2) + 理想位置相对偏差(2) + 无人机索引(1)
    ...
    def load_models(self):
        model_dir = "models-gnn-mp-7355-0个障碍物"  # 修改模型目录名称
        try:
...
def evaluate(env, agent, num_episodes=10, seed=42, render_images=False, writer=None):
    ...
    episode_lengths = []
    episode_collisions = []  # 记录每个episode的碰撞次数
    episode_formation_offsets = []  # 记录每个episode的平均编队偏移量
    ...
    for ep in range(num_episodes):
        ...
        episode_length = 0
        episode_collision = 0  # 当前episode的碰撞次数
        episode_formation_offset = 0.0  # 当前episode的编队偏移量累计
        formation_steps = 0  # 形成编队后的步数
        ...
        while True:
            ...
            for i in range(env.num_uavs):
                trajectories[i].append(env.uav_positions[i].copy())
            ...
            if env.initial_formation_reached:
                ...
                episode_formation_offset += formation_offset
                formation_steps += 1
            ...
            if done:
                # 计算平均编队偏移量
                avg_formation_offset = episode_formation_offset / formation_steps if formation_steps > 0 else 0.0
        ...
        # 记录到TensorBoard
        if writer:
            writer.add_scalar('eval/ep_rew', np.mean(episode_reward), ep)
            writer.add_scalar('eval/ep_len', episode_length, ep)
            writer.add_scalar('eval/ep_collisions', episode_collision, ep)
            writer.add_scalar('eval/formation_offset', avg_formation_offset, ep)

        print(f"Episode {ep + 1}: steps={episode_length}, reward={np.mean(episode_reward):.2f}, "
              f"collisions={episode_collision}, formation_offset={avg_formation_offset:.4f}, "
    ...
    avg_formation_offset = np.mean(episode_formation_offsets)

    # 记录平均指标到TensorBoard
    if writer:
        writer.add_scalar('eval/avg_rew', avg_reward, num_episodes)
        writer.add_scalar('eval/avg_len', avg_length, num_episodes)
        writer.add_scalar('eval/avg_collisions', avg_collisions, num_episodes)
        writer.add_scalar('eval/avg_formation_offset', avg_formation_offset, num_episodes)

    return avg_reward, avg_length, avg_collisions, avg_formation_offset
