...
class GNNAgent:
    ...
    def load_models(self):
        model_dir = "models-gnn-mp-7355-3个障碍物修2"
        try:
...
# train_formation.py (Partial Update - replace the evaluate function)

# train_formation.py (仅更新 evaluate 函数)

def evaluate(env, agent, num_episodes=10, seed=42, save_step_images=False, writer=None, global_step=0):
    """
    Args:
        global_step: 传入的基础步数，通常为0，或者当前训练的step
    """
    ...
    if save_step_images:
        ...
        print(f"Rendering enabled. Images will be saved to {step_images_dir}")
    ...
    for ep in range(num_episodes):
        ...
        while True:
            ...
            # 计算编队偏差
            current_step_dev = 0.0
            for i in range(env.num_uavs):
                dist = np.linalg.norm(env.uav_positions[i] - env.ideal_positions[i])
                current_step_dev += dist
            avg_step_dev = current_step_dev / env.num_uavs
            step_deviations.append(avg_step_dev)
        ...
        # --- 关键修改：每个 Episode 结束后立即写入 Tensorboard ---
        ep_avg_reward = np.mean(episode_reward)
        ep_avg_dev = np.mean(step_deviations)

        episode_rewards.append(ep_avg_reward)
        episode_lengths.append(episode_length)
        episode_collisions.append(episode_collision)
        episode_deviations.append(ep_avg_dev)

        if writer is not None:
            # 使用 (global_step + ep) 作为 x 轴，这样会画出一条线
            current_idx = global_step + ep
            writer.add_scalar('Eval_Detail/Reward', ep_avg_reward, current_idx)
            writer.add_scalar('Eval_Detail/Length', episode_length, current_idx)
            writer.add_scalar('Eval_Detail/Collisions', episode_collision, current_idx)
            writer.add_scalar('Eval_Detail/Deviation', ep_avg_dev, current_idx)
            writer.flush()  # 强制写入磁盘

        print(f"Episode {ep + 1}/{num_episodes}: Reward={ep_avg_reward:.2f}, "
              f"Collisions={episode_collision}, Deviation={ep_avg_dev:.2f}")
    ...
    # 记录总平均值（作为一个单独的点，或者你可以不用记录这个，如果只看细节的话）
    if writer is not None:
        writer.add_scalar('Eval_Summary/Avg_Reward', avg_reward, global_step)
        writer.add_scalar('Eval_Summary/Avg_Collisions', avg_collisions, global_step)
        writer.add_scalar('Eval_Summary/Avg_Deviation', avg_deviation, global_step)
        writer.flush()  # 强制写入磁盘

    return avg_reward, avg_length, avg_collisions, avg_deviation
