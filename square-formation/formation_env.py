# formation_env.py - 4架无人机动态障碍物版本 (带障碍物轨迹绘制)
...
class MultiUAVGymEnv(gym.Env):
    def __init__(self, world_size=1000, step_size=15.0, max_steps=600,
        ...
        self.obstacle_positions = np.array([
            [580, 450],  # 障碍物1初始位置 - 路径中间
            [230, 400],  # 障碍物2初始位置 - 靠近目标
            [630, 630]  # 障碍物3初始位置 - 路径侧方
        ], dtype=np.float32)
        ...
        # 定义障碍物运动区域 [min_x, min_y, max_x, max_y]
        self.obstacle_areas = [
            [530, 400, 630, 500],  # 障碍物1的运动区域 - 路径中间区域
            [180, 350, 280, 450],  # 障碍物2的运动区域 - 目标前方区域
            [580, 580, 680, 680]  # 障碍物3的运动区域 - 侧方干扰区域
        ]
        ...
        self.full_trajectories = None  # 记录无人机完整轨迹
    ...
    def save_trajectory_image(self, episode, steps):
        ...
        labels = ['Agent 0', 'Agent 1', 'Agent 2', 'Agent 3']
        uav_plots = []
        for i in range(self.num_uavs):
