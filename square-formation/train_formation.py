...
class GNNAgent:
    def __init__(self, env, gamma=0.99, gae_lambda=0.95,
        ...
        # 创建边索引 - 改为只连接邻居的边
        self.base_edge_index = self._create_neighbor_edges(env.num_uavs)
    ...
    def _create_neighbor_edges(self, num_nodes):
        """创建只连接邻居的边索引"""
        # 定义邻居关系 (方形编队)
        # 0连接1和3
        # 1连接0和2
        ...
        edge_index = torch.tensor(neighbor_pairs, dtype=torch.long).t().contiguous()
        return edge_index
    ...
    def load_models(self):
        model_dir = "models-方形-3动态"  # 修改模型目录名称
        try:
